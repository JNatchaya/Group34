...
function C_management_tab() {
  ...
  const [editingEntity, setEditingEntity] = useState({ type: null, data: null }); // Track editing entity
  ...
  return (
    <div className="c-management-container" onClick={handleCloseContextMenu}>
      ...
      {/* Main content: Companies or Departments */}
      {currentPage === "company" && (
        <div className="company-container">
          {combinedData.map((company, index) => (
            <div
              key={index}
              className={`account-item ${selectedCompany?.CMname === company.CMname ? "active" : ""}`}
              onClick={() => {
      ...
      {currentPage === "department" && selectedCompany && (
        <div className="company-container">
          {selectedCompany.DPCH.map((department, index) => (
            <div
              key={index}
              className={`account-item ${selectedDepartment?.DPName === department.DPName ? "active" : ""}`}
              onClick={() => setSelectedDepartment(department)}
              onContextMenu={(e) => handleRightClick(e, department, "department")}
            >
              <div className="company-logo"></div>
              <div className="company-name">
                {department.DPName}
      ...
      {/* Add buttons */}
      <button className="add-company-btn" onClick={currentPage === "company" ? handleAddCompany : handleAddDepartment}>
        ...
        <div className="popup-overlay">
          <div className="popup-container styled-popup">
            ...
            <form>
              ...
              <input type="text" className="form-input large-input" placeholder="Enter company name" />
              <label className="form-label">Date of subscription</label>
              <input type="date" className="form-input" />
              <button type="submit" className="form-submit-btn">Confirm</button>
            </form>
            <button className="close-popup-btn" onClick={closeCompanyPopup}>✕</button>
          </div>
        </div>
      )}
        ...
        <div className="popup-overlay">
          <div className="popup-container styled-popup">
            ...
            <form>
              ...
              <input type="text" className="form-input large-input" placeholder="Enter department name" />
              <label className="form-label">Location</label>
              <MapContainer center={[51.505, -0.09]} zoom={13} className="map" style={{ height: "300px", width: "100%" }}>
                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                <LocationSelector setLocation={setDepartmentLocation} />
                {departmentLocation && <Marker position={[departmentLocation.lat, departmentLocation.lng]} />}
              </MapContainer>
            ...
            <button className="close-popup-btn" onClick={closeDepartmentPopup}>✕</button>
          </div>
        </div>
      )}
        ...
        <div className="popup-overlay">
          <div className="popup-container">
            <h3 className="popup-header">
              Edit {editingEntity.type === "company" ? "Company" : "Department"} Information
            </h3>
            <form>
              <label className="form-label">
                {editingEntity.type === "company" ? "Company Name" : "Department Name"}
              </label>
              <input
                type="text"
                className="form-input"
                value={editingEntity.data?.CMname || editingEntity.data?.DPName || ""}
                onChange={(e) =>
                  setEditingEntity({
                    ...editingEntity,
                    data: {
                      ...editingEntity.data,
                      [editingEntity.type === "company" ? "CMname" : "DPName"]: e.target.value,
                    },
                  })
                }
              />
